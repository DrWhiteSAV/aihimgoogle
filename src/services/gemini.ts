...
export async function generateNewElement(
  ...
  const prompt = `
    ...
    ИСХОДНЫЕ ЭЛЕМЕНТЫ:
    1. ${elementA.name}: Состояние: ${elementA.state}, Темп: ${elementA.temperature}, Стабильность: ${elementA.stability}, Сущности: ${elementA.essences.join(', ')}, Слой: ${elementA.realityLevel}.
    2. ${elementB.name}: Состояние: ${elementB.state}, Темп: ${elementB.temperature}, Стабильность: ${elementB.stability}, Сущности: ${elementB.essences.join(', ')}, Слой: ${elementB.realityLevel}.
    ...
    ТВОЯ ЛОГИКА:
    1. Смешай параметры. Проверь законы (Температура, Противоположности, Жизнь, Время).
