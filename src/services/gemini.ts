...
export async function generateNewElement(
  ...
  const prompt = `
    ...
    ТЕКУЩИЙ СЛОЙ РЕАЛЬНОСТИ: ${currentLayer ? `${currentLayer.level} - ${currentLayer.name}` : '1 - Физический'}.
    ПРАВИЛА СЛОЯ: ${currentLayer ? currentLayer.rules : 'Базовые законы материи.'}
    
    ДОСТУПНЫЕ СЛОИ ДЛЯ ОТКРЫТИЯ: ${availableLayers.map(l => `${l.level} - ${l.name}`).join(', ')}
    ...
    ИСХОДНЫЕ ЭЛЕМЕНТЫ:
    1. ${elementA.name}: Тип: ${elementA.type}, Состояние: ${elementA.state}, Темп: ${elementA.temperature}, Стабильность: ${elementA.stability}, Сущности: ${elementA.essences.join(', ')}, Слой: ${elementA.realityLevel}.
    2. ${elementB.name}: Тип: ${elementB.type}, Состояние: ${elementB.state}, Темп: ${elementB.temperature}, Стабильность: ${elementB.stability}, Сущности: ${elementB.essences.join(', ')}, Слой: ${elementB.realityLevel}.
    ...
    - Космический: 2%
    - Изначальный: 0.9%
    - Трансцендентный: 0.09%
    - Запретный: 0.01% (Требует Аномалию!)
